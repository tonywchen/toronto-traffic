    ...
    return (
      <div
        ...
        <div className={`timeline-info flex h-12 w-full max-w-screen-sm rounded bg-blue-500 bg-opacity-25 text-gray-300 ${justifyClass} items-center`}>
          <div className={`px-3 lg:px-6 text-xs lg:text-sm`}>
            { message }
            { offer }
          </div>
          <button
            onClick={() => jumpToDate(dataStatus.last)}
            className={`h-full px-3 lg:px-6 rounded bg-blue-500 text-white text-xs lg:text-sm font-bold justify-center items-center ${hiddenClass}`}>
            { action }
          </button>
        </div>
      </div>
